...
describe("迁移：给老库补列", () => {
  it("缺 last_pass_session / points 时补上，已有时不动", () => {
    ...
    db.exec("CREATE TABLE session (id INTEGER PRIMARY KEY); CREATE TABLE card_state (item_id TEXT PRIMARY KEY, pass_streak INTEGER NOT NULL DEFAULT 0); CREATE TABLE chapter (id TEXT PRIMARY KEY)");
    expect(migrate(db)).toEqual(["card_state.last_pass_session", "chapter.points"]);
