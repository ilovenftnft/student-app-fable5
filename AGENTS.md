...
- 装载范围（2026-09-01 模拟）：全部 1194 条 FSRS 内容 → 150 天中位 14.1 分钟，不通过。**默认只装英语"本册新词"（206 词 + 65 张听写卡），小学段 349 词暂不装**（`npm run load` 默认；`--vocab all` 可全装）→ 中位 9.9、p90 12.6、最大 15.8，无超 20 分钟天，通过。
