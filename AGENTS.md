...
3. 色彩 ≤ 7 个 token，**固定米黄底，不跟随系统**（家长 2026-09-02 改定：09-01 的深色看着不舒服；孩子端与家长页同一套）：`bg` #F4EDDB（米黄，不用纯白）、`surface` #FDF9F0、`border` #E0D6C2、`text` #2B2622（暖深灰，不用纯黑）、`text-muted` #6F6557、`accent` #267552（只用于进度、答案文字（填进空里的和答案卡里的都算，回想要点里加粗的概念词也算，规则在 `shared/emphasis.ts`）、折线；**按钮不用绿**，主按钮与次按钮同为 `surface` 底 + `border` 边框，只靠字重区分，家长 09-02 定）、`weak` #B04F4F（只用于错题边框）、`label` #9A5B2B（只用于讲解里的"第 N 步 / 关键一步 / 常见错因"小标题，家长 09-02 加，因此 token 共 7 个）。文字与底的对比度全部 ≥ 4.5:1。禁止渐变与装饰性色彩。视觉稿：`docs/design/`（画板文件已按米黄底改色；线上画布仍是深色旧稿）。
