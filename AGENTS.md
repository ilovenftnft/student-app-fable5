...
| # | 功能 | 一句话验收标准 |
|---|---|---|
| 1 | 每日闭环页 | 孩子从打开到结束页全程只按主按钮就能走完：开场（先做哪科 / 只做到期的或再多 5 张）→ 章节勾选 → 引导式回想 → 到期卡 → 三问（点选）→ 结束页；无照片、无 LLM 时全程可用；正计时只显示分钟；60 分钟硬停 |
...
勾选"今天学到 §X"后：显示节名，"这一节讲了什么？先想 1 分钟"（正计时，不锁按钮）→ 点"想好了"→ 展开该节要点（`chapter.points`）→ 勾"没想起来的"→ 写入 `recall`，勾中的要点次日在回想页顶部"先看一眼"（不进 FSRS、不占 10 分钟预算）。**默想，不打字，不计十分钟，不强制**；没勾章节时跳过这一步。依据：`docs/增补清单.md` 第二节第 9 条（儿童证据支持引导式检索而非空白自由回忆；默想与写出等效）。
...
3. 色彩 ≤ 6 个 token，**固定深色，不跟随系统**（家长 2026-09-01 定稿，孩子端与家长页同一套）：`bg` #121417、`surface` #1C2027、`border` #2A2F36、`text` #E8E8E4（灰白，不用纯白，减轻散光光晕）、`text-muted` #7D8590、`accent` #7EE2A8（只用于主按钮、进度、填空、折线）、`weak` #B85C5C（只用于错题边框）。禁止渐变与装饰性色彩。视觉稿：`docs/design/`（画布第一页为定稿）。
...
8. 反馈文案一句话、陈述式、无感叹号、无表情：正确"对了。"，错误"再看一眼第 2 步。"，结束"今天 7 题，2 张卡明天到期。"（第二句按数据拼，见开场页一节）。主按钮上写清下次什么时候："会 · 下次 12 天后 / 不会 · 下次 明天"（FSRS 预演，`scheduler/preview.ts`）。
9. 家长页同一视觉系统（同样固定深色），信息密度可更高（表格 + 一张折线图），不显示逐题与时长；讲解只给本周总次数。
