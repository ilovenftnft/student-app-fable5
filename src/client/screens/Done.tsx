    ...
    <section className="screen" style={{ minHeight: 0 }}>
      ...
      {today.previewLines.length > 0 && (
        <div style={{ marginTop: 48 }}>
          <p className="t-tag" style={{ margin: "0 0 10px" }}>下次上课</p>
          <p style={{ margin: 0, fontSize: 19, lineHeight: "30px" }}>{today.previewLines[0]}</p>
          {today.previewLines.slice(1).map((l, i) => <p key={i} className="muted" style={{ margin: "6px 0 0", fontSize: 17, lineHeight: "26px" }}>{l}</p>)}
        </div>
      )}
