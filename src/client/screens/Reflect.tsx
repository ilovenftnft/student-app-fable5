...
export function Reflect({ onDone }: { onDone: (t: Today) => void }) {
  ...
  return (
    <section className="fade" key={q.key}>
      <p className="t-small muted num" style={{ margin: 0 }}>{i + 1} / 3</p>
      <h1 className="t-title" style={{ margin: "8px 0 16px" }}>{q.text}</h1>
      ...
      <button className="btn-text" onClick={() => pick(null)}>跳过</button>
    </section>
  );
}
